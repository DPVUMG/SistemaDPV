<div class="card">
    <div class="card-header card-header-info">
        <h4 class="card-title">{{ __('Pagos registrados') }}</h4>
        <p class="card-category">{{ "En esta sección se muestran todos los pagos registrados en el sistema." }}</p>
    </div>
    <div class="card-body">
                    ...
                    <tr>
                        ...
                        <td class="text-center align-middle">
                            <a href="{{ route('escuela_pedido.show', $item->escuela_pedido_id) }}" target="_blank"
                                title="Ver información del pedido {{ $item->escuela_pedido_id }}"
                        ...
                        <td class="text-center align-middle">
                            <form id="formAnular{{ $item->id }}" method="post"
                                action="{{ route('pago.destroy', $item) }}">
                                @csrf
                                @method('delete')
                                <button rel="tooltip" data-toggle="tooltip" data-placement="top"
                                    title="Anular el pago número {{ $item->id }}"
                                    class="btn btn-danger btn-sm btnAnular" id="btnAnular-{{ $item->id }}">
